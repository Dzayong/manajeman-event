...
import { renderReportHtml } from "@/lib/report-markdown";
...
export function ReportWorkspace({
  ...
  const docTitle = type === "LPJ_DRAFT" ? "Draft LPJ" : "Ringkasan Progress";

  // Rendered from live editor state, so an export always matches what the
  // user is looking at -- including edits not saved to the server yet.
  const renderedHtml = useMemo(() => renderReportHtml(content), [content]);

  function handleDownloadDoc() {
    const safeEvent = eventName.replace(/[^a-zA-Z0-9]+/g, "-");
    downloadAsDoc({
      html: renderedHtml,
      title: docTitle,
      eventName,
      fileName: `${docTitle.replace(/\s+/g, "-")}-${safeEvent}`,
      isLpj: type === "LPJ_DRAFT",
      ketuaPelaksana,
    });
    toast.success("File .doc diunduh — bisa langsung diedit di Word");
  }
      ...
      <>
        ...
        {editable && (
          <form
            ...
            className="mt-3 flex shrink-0 items-end gap-2 border-t pt-3"
          >
            <Textarea
              value={instruction}
  ...
  return (
    ...
    <div className="grid gap-6 lg:grid-cols-2 lg:items-start print:hidden">
      ...
      {/* Desktop: chat panel pinned beside the draft, own scroll, doesn't stretch to match draft height */}
      <Card className="hidden lg:sticky lg:top-20 lg:flex lg:max-h-[calc(100vh-6rem)] lg:flex-col">
        <CardHeader>
      ...
      {/* Tablet & mobile: chat collapses into a floating icon that pops up a bottom sheet */}
      <div className="lg:hidden">
        <Sheet open={chatOpen} onOpenChange={setChatOpen}>
          <SheetTrigger asChild>
            <Button
              size="icon"
              className="fixed right-5 bottom-5 z-40 h-14 w-14 rounded-full shadow-lg"
              aria-label="Ngobrol dengan AI"
            >
          ...
          <SheetContent
            side="bottom"
            className="flex flex-col p-0 data-[side=bottom]:h-[85vh]"
          >
    ...
    <ReportPrintView
      ...
      eventName={eventName}
      createdAt={createdAt}
      photos={type === "LPJ_DRAFT" ? photos : { receipts: [], documentation: [] }}
      isLpj={type === "LPJ_DRAFT"}
